...
from data.loadGowalla import train_pair_sampling, train_neg_sampling, test_neg_sampling
############################################ Train ####################################################
...
#####################
def train_neg_sample(model, batch_size, train_df, train_pos_neg, adj, optim, lossfn, is_parallel):
    model.train()
    total_loss = 0.0
    ...
    for batch_id in range(n_batches):
        ...
        batch_negsampled = train_neg_sampling(batch_train_df, train_pos_neg)

        userIdx = torch.LongTensor(batch_negsampled['userId'].values)
        pos_itemIdx = batch_negsampled['itemId'].values
        neg_itemIdxs = batch_negsampled['neg_samples'].values

    # for batch_id, (userIdx, pos_itemIdx, neg_itemIdxs) in enumerate(train_loader):
        # userIdx = userIdx.long().cuda() 
        pos_itemIdx = torch.LongTensor(pos_itemIdx.reshape(-1,1)) # (batch_size, 1)  
        neg_itemIdxs = torch.LongTensor(np.stack(neg_itemIdxs))  # (batch_size, 4)     
        itemIdxs = torch.cat([pos_itemIdx, neg_itemIdxs], dim=1) # (batch_size, 5)
...
def train_bpr(model, batch_size, train_df, train_pos_neg, optim, lossfn):
    # lossfn = BPRLoss()
    model.train()
    ...
    for batch_id in range(n_batches):
        ...
        pos_item_idxs = batch_pairsampled['pos_sample'].values
        neg_item_idxs = batch_pairsampled['neg_sample'].values
...
def eval_neg_sample(model, batch_size, test_df, test_pos_neg, adj, top_k, is_parallel):
    HR, NDCG = [], []
    ...
    with torch.no_grad():
        ...
        n_batches = len(test_df) // batch_size + 1
        ...
        for batch_id in range(n_batches):
            ...
            batch_negsampled, _ = test_neg_sampling(batch_test_df, test_pos_neg)
        ...
        # for batch_id, (userIdx, pos_itemIdx, neg_itemIdxs) in enumerate(test_loader):
            ...
            # userIdx = userIdx.long().cuda()     
            pos_itemIdx = torch.LongTensor(pos_itemIdx.reshape(-1,1)) # (batch_size, 1)         
            neg_itemIdxs = torch.LongTensor(np.stack(neg_itemIdxs))  # (batch_size, 99)    
            ...
            if batch_id % 240 == 0 :
                print("-----------The timeStamp of evaluating batch {:03d}/{}".format(batch_id, n_batches) + " is: " + time.strftime("%H: %M: %S", time.gmtime(time.time())))
        pool.close()
    return np.mean(HR), np.mean(NDCG)
...
def eval_neg_all(model, batch_size, test_df, test_pos_neg, itemNum, is_parallel):
    ...
    for batch_id in range(n_user_batchs):
        ...
        userIdx = torch.tensor(batch_test_df['userId'].values)
        pos_itemIdxs = batch_test_df['positive_items'].values
        neg_itemIdxs = batch_test_df['negative_items'].values
